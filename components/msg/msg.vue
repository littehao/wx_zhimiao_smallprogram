<template>
	<tui-swipe-action class="msgCard" backgroundColor="#fff" :operateWidth="100" :forbid="forbid">
		<template v-slot:content>
			<view class="msg-item-content flex align-center px-32" :class="top?'bgF2F3F5':''" @click="$emit('click')">
				...
				<view v-else class="position-relative mr-2">
				    ...
				    <image v-else class="rounded-circle" src="/static/images/avatar.png" mode="aspectFill"  style="width:40px;height:40px;"></image>
				...
				<view class="flex flex-column flex-1">
					<view class="flex align-center mb-1">
						...
						<text class="fs-24 ft86909C">{{msgTime}}</text>
					</view>
					<view class="flex align-center">
						<text class="fs-24 ft4E5969 text-ellipsis flex-1 pr-2">{{msgContent}}</text>	
		...
		<template v-slot:button>
			<view class="msg-item-btn flex align-center pl-2" :class="top?'bgF2F3F5':''">
				<view class="tui-custom-btn tui-custom-mr mr-2" @tap="customBtn(0)">
					<image v-if="top" src="/static/images/msgbottom.png" style="width: 32px;height: 32px;" mode="widthFix"></image>
					<image v-else src="/static/images/msgtop.png" style="width: 32px;height: 32px;" mode="widthFix"></image>
				</view>
				<view class="tui-custom-btn" @tap="customBtn(1)">
